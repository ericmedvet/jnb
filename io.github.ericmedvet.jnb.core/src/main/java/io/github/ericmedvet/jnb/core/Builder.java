...
public interface Builder<T> {
  ...
  /// Builds an object of type `T` from a `ParamMap` and a `NamedBuilder`. This method calls the
  /// overloaded `build` method with an index of 0.
  ///
  ...
  /// building sequences and is expected to be set accordingly by the caller.
  ///
  /// @param map          the `ParamMap` containing the parameters for building the object
  /// @param namedBuilder the `NamedBuilder` that can be used to recursively build values in the
  ///                     `ParamMap`
  ...
  /// Returns a cached version of this `Builder`. The cached builder stores previously built objects
  /// in a cache, so that if the same `ParamMap` is provided again, the cached object is returned
  /// instead of building a new one. Internally, the cache is a `WeakHashMap`; the index is ignored
  /// when checking for the presence of a map in the cache.
  ///
