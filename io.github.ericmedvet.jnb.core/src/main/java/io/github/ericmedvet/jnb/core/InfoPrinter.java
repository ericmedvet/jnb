...
public class InfoPrinter {
  ...
  public InfoPrinter(
      int packageHeadingLevel, int builderHeadingLevel, boolean shortenFQNs, boolean computeCompatibilities
  ) {
  ...
  record BuilderInfo(SortedSet<Name> names, Builder<?> builder, List<ParamCompatibility> compatibilities,
                     SortedSet<String> workingUsages) {
    public Name longestName() {
      return names().stream().max(Comparator.comparingInt(n -> n.fullName().length())).orElseThrow();
    }

    public Name shortestName() {
      return names().stream().min(Comparator.comparingInt(n -> n.fullName().length())).orElseThrow();
    }
  }
  ...
  private static List<ParamTriplet> easyParamPairs(List<DocumentedBuilder.ParamInfo> builder) {
    return builder.stream()
        ...
        .map(pi -> new ParamTriplet(pi.name(), switch (pi.type()) {
          case INT -> "1";
          case DOUBLE -> "1.0";
          case STRING -> "a";
          case BOOLEAN -> "false";
    ...
    if (!builderInfos.isEmpty() && !npmParams.isEmpty()) {
      ...
      while (!done) {
        ...
        for (int i = 0; i < indexes.length; i++) {
          localParamTriplets.add(new ParamTriplet(npmParams.get(i).name(),
  ...
  public void print(NamedBuilder<?> nb, PrintStream ps) {
    ...
    builders.forEach((fullName, builder) -> {
      Name mainName = aliasesMap.entrySet()
          .stream()
      ...
      if (mainName == null) {
        ...
        aliasesMap.put(
            new Name(fullName),
            new BuilderInfo(new TreeSet<>(), builder, Collections.unmodifiableList(compatibilities), new TreeSet<>())
        );
    ...
    if (computeCompatibilities) {
      ...
      int nOfAttempts = 0;
      while (aliasesMap.values()
          .stream()
          .anyMatch(bi -> bi.workingUsages().isEmpty()) && nOfAttempts < N_OF_COMPATIBILITY_ATTEMPTS) {
        nOfAttempts = nOfAttempts + 1;
        System.out.printf("Usages attempt %d on %d: %d known, %d unknown%n",
            nOfAttempts,
        ...
        for (BuilderInfo bi : aliasesMap.values()) {
          if (bi.workingUsages().isEmpty()) {
            if (bi.builder() instanceof DocumentedBuilder<?> builder) {
              ...
              System.out.printf("Usages for %s with %d cases%n", bi.shortestName().fullName(), triplets.size());
              for (List<ParamTriplet> paramTriplets : triplets) {
                try {
                  String usage = "%s(%s)".formatted(bi.shortestName().fullName(),
      ...
      for (BuilderInfo builderInfo : buildableBuilders) {
        if (builderInfo.builder() instanceof DocumentedBuilder<?> builder) {
          ...
          List<List<ParamTriplet>> triplets = paramTriplets(builder, buildableBuilders);
          System.out.printf("Compatibility for %s with %d cases%n",
              builderInfo.shortestName().fullName(),
              triplets.size()
          );
          for (List<ParamTriplet> paramTriplets : triplets) {
            try {
              String usage = "%s(%s)".formatted(builderInfo.shortestName().fullName(),
                  paramTriplets.stream()
    ...
    aliasesMap.forEach((name, builderInfo) -> {
      String longestPackageName = builderInfo.names()
          .stream()
          .map(Name::packageName)
          .max(Comparator.comparingInt(String::length))
          .orElseThrow();
      PackageInfo packageInfo = packagesMap.getOrDefault(longestPackageName,
              ...
              if (documentedBuilder.params().stream().noneMatch(p -> p.injection().equals(Param.Injection.NONE))) {
                ...
                for (ParamCompatibility compatibility : builderInfo.compatibilities()) {
                  if (compatibility.paramInfo().injection().equals(Param.Injection.NONE)) {
                    ps.printf("| `%s` | %s | %s | %s |" + (computeCompatibilities ? " %s|" : "") + "%n",
  ...
  private String shortenJavaTypeName(Type javaType) {
    ...
    String abbrName = typeName.replaceAll("([a-z][a-zA-Z0-9_.]+)\\.([a-zA-Z0-9$]+)", "<abbr title=\"$0\">$2</abbr>");
